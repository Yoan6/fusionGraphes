...
# Ville à rechercher
ville = "Moirans"  # TODO: Modifier pour prendre en compte le code de la commune (objet['isLocatedAt']['schema:address']['hasAddressCity']['@id'][3:])
...
if etablissements_ville_recherchee:
    ...
    def build_graph_data_tourisme(data):
        ...
        node_id_counter += 1
        G_economique.add_node(node_id_economique)
        node_labels[node_id_economique] = "Tourisme et loisir"
        node_details[node_id_economique] = {'title': "Tourisme et loisir", 'texte': "", 'balise': "h3"}

        node_id_culturel = node_id_counter
        node_id_counter += 1
        G_culturel.add_node(node_id_culturel)
        node_labels[node_id_culturel] = "Patrimoine culturel"
        node_details[node_id_culturel] = {'title': "Patrimoine culturel", 'texte': "", 'balise': "h3"}
        ...
        def add_nodes():
            ...
            if ('schema:FoodEstablishment' or 'schema:LocalBusiness' or 'schema:Hotel' or 'schema:LodgingBusiness' or 'schema:Accommodation' or 'schema:TouristInformationCenter' or 'schema:Winery' or 'schema:Restaurant' or 'schema:Product') in objet['@type']:
                ...
                node_details[node_id_etablissement] = {'title': title, 'text': "", 'balise': "h4"}
                ...
                if 'owl:topObjectProperty' in objet and objet['owl:topObjectProperty']['shortDescription']['@value']:
                    ...
                    node_details[node_id_description] = {'title': "Description", 'text': objet['owl:topObjectProperty']['shortDescription']['@value'], 'balise': "p"}
                ...
                if ('schema:geo' in objet['isLocatedAt'] and 'schema:latitude' in objet['isLocatedAt']['schema:geo'] and 'schema:longitude' in objet['isLocatedAt']['schema:geo']):
                    ...
                    node_details[node_id_coord] = {'title': "Coordonnées", 'text': str(objet['isLocatedAt']['schema:geo']['schema:latitude']['@value']) + ", " + str(objet['isLocatedAt']['schema:geo']['schema:longitude']['@value']), 'balise': "p"}
    ...
    G_economique, G_culturel, node_labels, edge_labels, node_details = build_graph_data_tourisme(etablissements_ville_recherchee)
